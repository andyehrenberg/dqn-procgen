...
from level_replay.algo.dqn_agent import Agent
...
def train(args, seeds):
    ...
    for j in range(num_updates):
        ...
        if (j % args.log_interval == 0 and len(episode_rewards) > 1) or j == num_updates - 1:
            ...
            eval_episode_rewards = evaluate(args, model, args.num_test_seeds, device)

            logging.info(f"\nEvaluating on {args.num_test_seeds} train levels...\n  ")
            train_eval_episode_rewards = evaluate(args, model, args.num_test_seeds, device, start_level=0, num_levels=args.num_train_seeds, seeds=seeds)
            ...
            if j == num_updates - 1:
                ...
                final_eval_episode_rewards = evaluate(args, model, args.final_num_test_seeds, device)
