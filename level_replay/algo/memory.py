...
Transition_dtype = np.dtype([('timestep', np.int32), ('obs', (np.uint8, (3, 64, 64))), 
...
class ReplayMemory():
    ...
    def compute_returns(self,
                        ...
                        gamma,
                        gae_lambda):
        self.transitions.data['value_pred'][-1] = next_value
        gae = 0
        for step in reversed(range(len(self['reward']))):
            delta = self.transitions.data['reward'][step] + gamma * self.transitions.data['value_pred'][
                step + 1] * self.transitions.data['mask'][step + 1] - self.transitions['value_pred'][step]
            gae = delta + gamma * gae_lambda * self.transitions.data['mask'][step +1] * gae
            self.transitions.data['return'][step] = gae + self.transitions.data['value_pred'][step]
    ...
    def __next__(self):
        ...
        transitions[blank_mask] = blank_trans
        state = torch.tensor(transitions['obs'], dtype=torch.float32, device=self.device).div_(255)  # Agent will turn into batch
        self.current_idx += 1
        return state