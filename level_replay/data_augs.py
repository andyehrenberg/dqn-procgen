...
class Cutout(object):
    ...
    def __init__(self, batch_size, box_min=7, box_max=22, pivot_h=12, pivot_w=24, *_args, **_kwargs):
...
class CutoutColor(object):
    ...
    def __init__(
        self, batch_size, box_min=7, box_max=22, pivot_h=12, pivot_w=24, obs_dtype="uint8", *_args, **_kwargs
    ):
        ...
        self.w1 = np.random.randint(self.box_min, self.box_max, batch_size)
        self.h1 = np.random.randint(self.box_min, self.box_max, batch_size)
        self.rand_box = np.random.randint(0, 255, size=(batch_size, 1, 1, 3), dtype=obs_dtype) / 255.0
...
class Flip(object):
    ...
    def __init__(self, batch_size, p_rand=0.5, *_args, **_kwargs):
        ...
        self.p_flip = p_rand
        self.batch_size = batch_size
        self.random_inds = np.random.choice([True, False], batch_size, p=[self.p_flip, 1 - self.p_flip])
    ...
    def change_randomization_params(self, index_):
        self.random_inds[index_] = np.random.choice([True, False], 1, p=[self.p_flip, 1 - self.p_flip])

    def change_randomization_params_all(self):
        self.random_inds = np.random.choice([True, False], self.batch_size, p=[self.p_flip, 1 - self.p_flip])
...
class Rotate(object):
    ...
    def change_randomization_params_all(self):
        self.random_inds = np.random.randint(4, size=self.batch_size) * self.batch_size + np.arange(
            self.batch_size
        )
...
class Crop(object):
    ...
    def do_augmentation(self, x):
        aug_trans = nn.Sequential(nn.ReplicationPad2d(12), kornia.augmentation.RandomCrop((64, 64)))
        return aug_trans(x)
...
class ColorJitter(nn.Module):
    ...
    def adjust_contrast(self, x):
        ...
        return torch.clamp((x - means) * self.factor_contrast.view(len(x), 1, 1, 1) + means, 0, 1)

    def adjust_hue(self, x):
        h = x[:, 0, :, :]
        h = h + (self.factor_hue.view(len(x), 1, 1) * 255.0 / 360.0)
        h = h % 1
        x[:, 0, :, :] = h
        return x
    ...
    def adjust_brightness(self, x):
        ...
        Returns:
            torch tensor image: Brightness adjusted
        """
        x[:, 2, :, :] = torch.clamp(x[:, 2, :, :] * self.factor_brightness.view(len(x), 1, 1), 0, 1)
    ...
    def adjust_saturate(self, x):
        ...
        Returns:
            torch tensor image: Brightness adjusted
        """
        x[:, 1, :, :] = torch.clamp(x[:, 1, :, :] * self.factor_saturate.view(len(x), 1, 1), 0, 1)
        return torch.clamp(x, 0, 1)

    def transform(self, inputs):
        # Shuffle transform
        if random.uniform(0, 1) >= 0.5:
            inputs = self.adjust_contrast(inputs)
            inputs = self.rgb2hsv(inputs)
            inputs = self.adjust_brightness(inputs)
            inputs = self.adjust_hue(inputs)
            inputs = self.adjust_saturate(inputs)
            inputs = self.hsv2rgb(inputs)
            return inputs
        else:
            inputs = self.rgb2hsv(inputs)
            inputs = self.adjust_brightness(inputs)
