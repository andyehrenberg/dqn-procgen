...
def evaluate_saved_model(
    ...
    if args.xpid is None:
        ...
        checkpointpath = os.path.expandvars(os.path.expanduser(os.path.join(result_dir, xpid, "model.tar")))
    ...
    level_sampler = LevelSampler(
        seeds, dummy_env.observation_space, dummy_env.action_space, strategy="sequential"
    )
    ...
    logging.info("Average returns over %i episodes: %.2f", num_episodes, mean_return)
    logging.info("Median returns over %i episodes: %.2f", num_episodes, median_return)
